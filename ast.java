...
class ExpListNode extends ASTnode {
    ...
    List<Type> getExpList() {
        List<Type> expList = new ArrayList<>();
        for (ExpNode node : myExps) {
            expList.add(node.typeCheck());
        }
        return expList;
    }
...
abstract class ExpNode extends ASTnode {
    ...
    // lineNum and charNum are used to get the position of the error
...
class IntLitNode extends ExpNode {
    ...
    // to get the position of the error
    public int lineNum() {
        return myLineNum;
    }
    public int charNum() {
...
class AssignNode extends ExpNode {
    ...
    // to get the postion of the error
    public int lineNum() {
    ...
    public Type typeCheck() {
        ...
        else if (left.isFnType()) {
            ErrMsg.fatal(myLhs.lineNum(), myLhs.lineNum(), "Function assignment");
            error = new ErrorType();
        }
        else if (left.isStructDefType()) {
            ErrMsg.fatal(myLhs.lineNum(), myLhs.lineNum(), "Struct name assignment");
            error = new ErrorType();
        }
        else if (left.isStructType()) {
            ErrMsg.fatal(myLhs.lineNum(), myLhs.lineNum(), "Struct variable assignment");
            error = new ErrorType();
        }
        else {
            return left;
...
class PlusNode extends BinaryExpNode {
    ...
    public Type typeCheck() {
        Type checkExp1 = myExp1.typeCheck();
        Type checkExp2 = myExp2.typeCheck();
        Type type = new IntType();

        if (checkExp1.isErrorType() || checkExp2.isErrorType()) {
            return new ErrorType();
        }

        if (!checkExp1.isIntType()) {
            ErrMsg.fatal(myExp1.lineNum(), myExp1.charNum(), "Arithmetic operator applied to non-numeric operand");
            type = new ErrorType();
        }

        if (!checkExp2.isIntType()) {
